...
import { getPluginCapabilities } from 'nx/src/utils/plugins/plugin-capabilities';
...
async function addPresetGenerator(
  ...
  const plugin = await getPluginCapabilities(workspaceRoot, schema.project);
  if (!plugin?.generators['preset']) {
